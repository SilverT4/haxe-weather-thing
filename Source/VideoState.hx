...
class VideoState extends FlxState
{
	...
	{
		if (FlxG.sound.music == null) FlxG.sound.playMusic(PathFinder.sound("theFunnyWell"));
		super.create();
	...
	{
		var okay = FlxG.keys.justPressed.ENTER || FlxG.keys.justPressed.SPACE;
		...
		if (okay || GlobalVideo.get().ended || GlobalVideo.get().stopped)
		{
			txt.visible = false;
